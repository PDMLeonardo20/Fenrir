...
let digitadores = [
    ...
    {nome: "JERUSA ARGUIRRE CARVALHO", setor: "ATENDIMENTO", cargo: "ATENDENTE II"},
...
for (let i = 0; i < cidade_agencia_redespacho.length; i++) {
    let bingus = document.createElement("option");
    bingus.value = cidade_agencia_redespacho[i].cidade;
...
function consulta_loc_redespacho(){
    ...
    for (let i = 0; i < cidade_agencia_redespacho.length; i++) {
        if (cidade_redespacho == cidade_agencia_redespacho[i].cidade) {
            document.getElementById("nome_agencia_redesp").value = cidade_agencia_redespacho[i].agencia;
            document.getElementById("uf_redesp").value = cidade_agencia_redespacho[i].estado;
        }
...
function consulta_redesp_por_cidade() {
    let agencia_redespachante = document.getElementById("nome_agencia_redespachante").value;

    document.getElementById("resultado_cidades_ate_redesp").innerHTML = "";

    for(let i = 0; i < cidade_agencia_redespacho.length; i++){
        if (agencia_redespachante == cidade_agencia_redespacho[i].agencia) {
            document.getElementById("resultado_cidades_ate_redesp").innerHTML += "<div class='bg-primary-subtle p-3 rounded-3 row'><p class='d-inline p-2 text-bg-primary rounded-3 m-3 col'>Cidade: <span><b>"+cidade_agencia_redespacho[i].cidade+"</b></span></p><p class='d-inline p-2 text-bg-primary rounded-3 m-3 col'>Estado: <span><b>"+cidade_agencia_redespacho[i].estado+"</b></span></p><p class='d-inline p-2 text-bg-primary rounded-3 m-3 col'>Agência: <span><b>"+cidade_agencia_redespacho[i].agencia+"</b></span></p></div><hr><br>";
        }
    }
}
